...
import module
from module.modules.sparse_losses import SparsemaxLoss
from module.modules.sparse_activations import LogSparsemax
...
def build_loss_compute(model, tgt_field, opt, train=True):
    ...
    """
    device = torch.device("cuda" if module.utils.misc.use_gpu(opt) else "cpu")
    ...
    if opt.copy_attn:
        criterion = module.modules.CopyGeneratorLoss(
    ...
    if opt.copy_attn:
        compute = module.modules.CopyGeneratorLossCompute(
...
class LossComputeBase(nn.Module):
    ...
    def __call__(self,
        ...
        Returns:
            A tuple with the loss and a :obj:`module.utils.Statistics` instance.
        """
        ...
        batch_stats = module.utils.Statistics()
    ...
    def _stats(self, loss, scores, target):
        ...
        Returns:
            :obj:`module.utils.Statistics` : statistics for this batch.
        """
        pred = scores.max(1)[1]
        non_padding = target.ne(self.padding_idx)
        num_correct = pred.eq(target).masked_select(non_padding).sum().item()
        num_non_padding = non_padding.sum().item()
        return module.utils.Statistics(loss.item(), num_non_padding, num_correct)
...
class NMTLossCompute(LossComputeBase):
    ...
    def _make_shard_state(self, batch, output, range_, attns=None):
        ...
        if self.lambda_align != 0.0:
            ...
            ref_align = module.utils.make_batch_align_matrix(
