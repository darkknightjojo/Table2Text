...
import module.model_builder
import module.inputters as inputters
from module.translate.beam_search import BeamSearch
from module.translate.greedy_search import GreedySearch
from module.utils.misc import tile, set_random_seed, report_matrix
from module.utils.alignment import extract_alignment, build_align_pharaoh
from module.modules.copy_generator import collapse_copy_scores
...
class Translator(object):
    ...
    Args:
        model (module.modules.NMTModel): NMT model to use for translation
        fields (dict[str, torchtext.data.Field]): A dict
            mapping each side to its list of name-Field pairs.
        src_reader (module.inputters.DataReaderBase): Source reader.
        tgt_reader (module.inputters.TextDataReader): Target reader.
        gpu (int): GPU device. Set to negative for no GPU.
        n_best (int): How many beams to wait for.
        min_length (int): See
            :class:`module.translate.decode_strategy.DecodeStrategy`.
        max_length (int): See
            :class:`module.translate.decode_strategy.DecodeStrategy`.
        beam_size (int): Number of beams.
        random_sampling_topk (int): See
            :class:`module.translate.greedy_search.GreedySearch`.
        ...
        global_scorer (module.translate.GNMTGlobalScorer): Translation
            ...
            if isinstance(self.model.decoder, StructureAwareDecoder) :
                if not len(rnn_weights) == len(self.model.decoder.rnn):
                    raise ValueError('Please specify exactly on weights per branch'
                                     ...
                                     f'{len(self.model.decoder.rnn)} branches.')
            else:
    ...
    def from_opt(
        ...
        Args:
            model (module.modules.NMTModel): See :func:`__init__()`.
            fields (dict[str, torchtext.data.Field]): See
                :func:`__init__()`.
            opt (argparse.Namespace): Command line options
            model_opt (argparse.Namespace): Command line options saved with
                the model checkpoint.
            global_scorer (module.translate.GNMTGlobalScorer): See
                :func:`__init__()`..
